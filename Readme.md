...
```bash
git clone https://github.com/itsbhavsagar/ai-companion.git
cd ai-companion
npm install
cp .env.example .env
...
- Broader semantic contradiction detection
...
## 📄 `ARCHITECTURE.md` — Final Clean Version

```markdown
# Architecture

This is a focused prototype of an AI companion memory system. The goal isn't to build a production social app — it's to prove the core loop: remember facts, retrieve the right ones, update stale ones, and keep a stable companion persona over long conversations.

The assignment explicitly scoped out UI, auth, billing, multi-user support, and production-scale infrastructure. Those are real concerns, but this repo concentrates on the memory architecture.
